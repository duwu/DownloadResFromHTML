...
//仅仅替换一次
void ReplaceStr(string&   str, string old_value, string new_value)
{
...
//计算某个字符或字符串在一个字符串里出现的额次数
int CountStrTimes(string SourceStr, string TagStr)
{
	int BeginCount = strlen(SourceStr.c_str());
	GlobalReStr(SourceStr, TagStr, "");
	int EndCount = strlen(SourceStr.c_str());
	return BeginCount - EndCount;
}


//对字符串进行异或加密或解密
string XOREncode(string SourceStr, int XORKeys[])
{
	//定义一个变量来存放结果
	string ResultStr = "";
	//对每个字节进行异或加密或解密
	for (size_t n = 0; n < SourceStr.length(); n++)
	{
		int A = (n) % 16;
		unsigned char TempC = (unsigned char)SourceStr.at(n);
		ResultStr += TempC^ XORKeys[A];
	}
	return ResultStr;
}
...
//得到文件后缀名
string GetNameExtensionFromPath(string PathStr)
{
	string FileExtendStr = "";
	size_t SpeicalCharIndex = PathStr.find_last_of(".");
	if (SpeicalCharIndex >= 0 && SpeicalCharIndex<PathStr.length())
	{
		FileExtendStr = PathStr.substr(PathStr.find_last_of("."));
	}
	return FileExtendStr;
}

//通过文件完整路径找到此文件或文件夹所在目录
string GetFileMDFromPath(string PathStr)
{
	string ResultStr = "";
	size_t SpeicalCharIndex = PathStr.find_last_of("\\/");
	size_t CharCount = PathStr.length();
	//找到字符串的话，则执行
	if (SpeicalCharIndex >= 0 && SpeicalCharIndex<CharCount)
	{
		ResultStr = PathStr.substr(0, PathStr.find_last_of("\\"));
	}
	return  ResultStr;
}

//设置一个路径下的同级文件名
string GetSiblingFilePath(string PathStr, string FileName)
{
	string ResultStr = "";
	string Slash = "/";
	string FileMD = GetFileMDFromPath(PathStr);

	//考虑到同时没有\\和/情况
	if (FileMD == string(""))
	{
		return FileName + GetNameExtensionFromPath(PathStr);
	}

	//考虑到..\..\..\目录
	if (PathStr.find_last_of("\\/") - PathStr.find_last_of(".") < PathStr.length())
	{
		return FileMD + Slash + FileName;
	}
	return FileMD + Slash + FileName + GetNameExtensionFromPath(PathStr);
}


//计算某个字符串在指定字符串里的出现次数
int GetShowTimesOfStr(string SourceStr, string SpecialStr)
{
	string TempStr = SourceStr;
	GlobalReStr(TempStr, SpecialStr, "");
	return SourceStr.length() - TempStr.length();
}



//仅仅基于完整路径来确定的文件的相对路径
string GetRelativePath(string CurrentPath, string WantPath)
{
	//判断两个字符串的相同部分
	string CommonStr = "";//存放两个字符串的共同部分

	int IsLoop = true;
	for (size_t i = 0; i < CurrentPath.length() && IsLoop; i++)
	{
		//逐个字符进行匹配
		if (CurrentPath.at(i) == WantPath.at(i))
		{
			CommonStr += CurrentPath.at(i);
		}
		else{
			IsLoop = false;
		}
	...
	//当存在下一个目录名仅仅是部分匹配时执行，将多余部分去掉
	if (CommonStr.find_last_of("\\/") + 1 != CommonStr.length())
	{
		CommonStr = CommonStr.substr(0, CommonStr.find_last_of("\\/") + 1);
	}

	//得到去除了相同路径后的信息
	string DifferentCurrentPath = CurrentPath.substr(CurrentPath.find(CommonStr) + CommonStr.length());
	string DifferentWantPath = WantPath.substr(WantPath.find(CommonStr) + CommonStr.length());

	//判断非相同路径上有多少个\\，从而获得需要替换的次数
	int CurrentTimes = GetShowTimesOfStr(DifferentCurrentPath, "\\");

	string FinalStr = DifferentWantPath.substr();

	//将所有不同目录用..\来替代
	for (int n = 0; n < CurrentTimes; n++)
	{
...
std::string GetApplicationDir()
{
	...
	path = path.substr(path.find_last_of("\\/") + 1);//这种字符串截取方式兼容了/和\的两种情况
	//path = path.substr(path.rfind("\\") + 1);//为了兼容可能出现/字符的路径，还需要path = path.substr(path.rfind("/") + 1)
	//cout << path << endl;
	return path;
}